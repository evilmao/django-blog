...
from django.core.paginator import Paginator, PageNotAnInteger, EmptyPage  #d导入分页模块    
'''
这是分页功能涉及的一些类和异常，官方文档对此有详细介绍。当然从命名也
可以直接看出它们的用途：Paginator（分页），PageNotAnInteger
（页码不是一个整数异常），EmptyPage（空的页码号异常）
'''
...
#使用装饰器表明函数是一个模板标签，参数takes_context = True表示接收上下文对象
def paginate(context,object_list,page_count=5):
    #定义函数，参数context是上下文需要传入的对象，object_list是要分页的对象，page_count表示每页的数量

    left = 3  # 当前页码左边西安市几个页码号 -1，比如3就是两个
    right = 3 # 当前页码右边西安市几个页码号 -1

    paginator = Paginator(object_list, page_count) # 通过object_list分页对象
    page = context['request'].GET.get('page')      # 从http请求中获取用户请求的页码数
    ...
    try:
       ...
       '''
        ...
        # 异常处理，如果用户传递的page值不是整数，
        object_list = paginator.page(1)   # 则把第一页的值返回给他
        context['current_page'] = 1       # 当前页是1
        pages = get_right(context['current_page'], right, paginator.num_pages)
    except EmptyPage:   
        # 如果用户传递的 page 值是一个空值
        object_list = paginator.page(paginator.num_pages) # 那么把最后一页的值返回给他
    ...
    try:
        ...
        注意这里可能产生异常，因为pages可能是一个空列表，比如本身只有一个分页，那么pages就为空，因为我们永远不会
        获取页码为1的页码号（至少有1页，1的页码号已经固定写在模板文件中）
        '''
...
def get_left(current_page, left, num_pages):
    ...
    if current_page == 1:
        ...
        l = [i - 1 for i in range(current_page, current_page - left, -1) if i - 1 > 1]  #使用列表推导式+if判断
        l.sort()   #使用sort排序 
        return l
...
def get_right(current_page, right, num_pages):
    """
    辅助函数，获取当前页码的值得右边两个页码值，要注意一些细节，比如不够两个那么最右取到最大页码值。不包含当前页
    码值。比如当前页码值为5，那么pages = [6,7]
    """
